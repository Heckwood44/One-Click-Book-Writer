...
class APIKeyChecker:
    ...
    def check_keys(self):
        ...
        if self.openai_key:
            ...
            if not self.openai_key or self.openai_key in ['your_openai_api_key_here', '']:
                self.openai_key = None
        ...
        if self.claude_key:
            ...
            if not self.claude_key or self.claude_key in ['your_anthropic_api_key_here', '']:
                self.claude_key = None
    ...
    def get_validation_report(self) -> Dict[str, Dict[str, any]]:
        ...
        report = {
            'openai': {
                ...
                'valid_format': self.validate_key_format(self.openai_key, 'openai') if self.openai_key else False,
                'key_preview': f"{self.openai_key[:10]}..." if self.openai_key else None
            },
            'claude': {
                'available': bool(self.claude_key),
                'valid_format': self.validate_key_format(self.claude_key, 'claude') if self.claude_key else False,
                'key_preview': f"{self.claude_key[:10]}..." if self.claude_key else None
            }
        }
        return report
    ...
    def can_generate_chapters(self) -> bool:
        ...
        return bool(self.openai_key and self.validate_key_format(self.openai_key, 'openai'))
    
    def can_optimize_prompts(self) -> bool:
        """Prüft ob Prompt-Optimierung möglich ist (benötigt Claude)"""
        return bool(self.claude_key and self.validate_key_format(self.claude_key, 'claude'))
    ...
    def print_status(self):
        ...
        if self.can_generate_chapters():
            ...
            print("❌ OpenAI (ChatGPT): Nicht verfügbar - Kapitelgenerierung nicht möglich")
