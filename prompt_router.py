...
class PromptRouter:
    ...
    def load_and_validate_prompt_frame(self, prompt_frame_path: str) -> Tuple[bool, Dict, str]:
    ...
    def optimize_prompt_with_claude(self, raw_prompt: str, prompt_frame: Dict) -> Tuple[bool, str, str]:
...
"""
    ...
    def generate_chapter_with_gpt(self, prompt: str, prompt_frame: Dict) -> Tuple[bool, str, str]:
        ...
        try:
            ...
            self.token_logger.log_api_call("openai", "gpt-4", estimated_tokens, "chapter_generation")
    ...
    def parse_bilingual_response(self, response: str) -> Tuple[str, str]:
        ...
        try:
            ...
            if len(parts) >= 2:
                ...
                german_part = re.sub(r'^#.*?\n', '', german_part, flags=re.MULTILINE).strip()
                german_part = re.sub(r'^DEUTSCH:\s*', '', german_part, flags=re.MULTILINE).strip()
                english_part = re.sub(r'^#.*?\n', '', english_part, flags=re.MULTILINE).strip()
                english_part = re.sub(r'^ENGLISH:\s*', '', english_part, flags=re.MULTILINE).strip()
    ...
    def run_full_pipeline(self, 
        ...
        try:
            ...
            is_valid, prompt_frame, message = self.load_and_validate_prompt_frame(prompt_frame_path)
            ...
            if optimize_with_claude:
                ...
                optimization_success, optimized_prompt, opt_message = self.optimize_prompt_with_claude(
                    raw_prompt, prompt_frame
                )
            ...
            generation_success, chapter_text, gen_message = self.generate_chapter_with_gpt(
                optimized_prompt, prompt_frame
            )
            ...
            metadata = self.save_output_files(german_text, english_text, prompt_frame, chapter_number)
            ...
            if quality_evaluation["overall_bilingual_score"] < 0.7:
                ...
                metadata["review_reason"] = f"Qualitäts-Score {quality_evaluation['overall_bilingual_score']} unter Schwellenwert 0.7"
                logger.warning(f"⚠️ Review erforderlich: Score {quality_evaluation['overall_bilingual_score']} < 0.7")
            
            if quality_evaluation["overall_bilingual_score"] < 0.5:
                metadata["critical_issues"] = True
                metadata["critical_reason"] = f"Kritische Qualitätsprobleme: Score {quality_evaluation['overall_bilingual_score']} < 0.5"
                logger.error(f"🚨 Kritische Probleme: Score {quality_evaluation['overall_bilingual_score']} < 0.5")
            ...
            try:
                ...
                if quality_evaluation["overall_bilingual_score"] < 0.7:
                    ...
                    existing_metadata["review_reason"] = f"Qualitäts-Score {quality_evaluation['overall_bilingual_score']} unter Schwellenwert 0.7"
            ...
            logger.info(f"Pipeline erfolgreich abgeschlossen für Kapitel {chapter_number}")
            logger.info(f"Qualitäts-Score: {quality_evaluation['overall_bilingual_score']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="One Click Book Writer - Prompt Router")
    ...
    parser.add_argument("--no-optimize", action="store_true", help="Claude-Optimierung überspringen")
