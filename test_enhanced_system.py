    ...
    for i, prompt_frame in enumerate(test_cases, 1):
        ...
        try:
            ...
            if result.ab_test_result:
                print(
                    f"   A/B Test Improvement: {result.ab_test_result.comparison.get('improvement_percentage', 0):+.1f}%"
                )

            # Zeige deutsche Version (gekürzt)
            german_preview = (
                result.generation_result.german_text[:200] + "..."
                if len(result.generation_result.german_text) > 200
                else result.generation_result.german_text
            )
...
def generate_final_report(results: List, pipeline):
    ...
    print(
        f"   Erfolgsrate: {(stats['successful_runs'] / max(stats['total_runs'], 1)) * 100:.1f}%"
    )
    print(
        f"   Durchschnittliche Ausführungszeit: {stats['average_execution_time']:.2f}s"
    )
    ...
    if successful_results:
        ...
        if ab_test_results:
            ...
            if ab_test_results:
                improvements = [
                    r.ab_test_result.comparison.get("improvement_percentage", 0)
                    for r in ab_test_results
                ]
