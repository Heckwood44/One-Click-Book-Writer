...
class SimpleBookWriterGUI:
    def __init__(self, root):
        ...
        if self.anthropic_api_key and len(self.anthropic_api_key.strip()) > 0:
            try:
                ...
                logger.error(
                    f"Claude Client Initialisierung fehlgeschlagen (Parameter/Import): {e}"
                )
                ...
                logger.exception(
                    f"Unerwarteter Fehler bei Claude Client Initialisierung: {e}"
                )
                self.claude_client = None
        ...
        self.example_data = {
            "input": {
                "book": {
                    ...
                    "titles": {"de": "Die magische Reise", "en": "The Magical Journey"},
                },
                "chapter": {
                    "number": 1,
    ...
    def setup_chapter_tab(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="One Click Book Writer - Kapitel Generierung",
            font=("Arial", 16, "bold"),
        )
        ...
        openai_status = (
            "✅ API Key verfügbar" if self.openai_api_key else "❌ API Key fehlt"
        )
        claude_status = (
            "✅ API Key verfügbar" if self.claude_client else "❌ API Key fehlt"
        )
        ...
        config_button = ttk.Button(
            api_frame, text="🔑 API Keys konfigurieren", command=self.configure_api_keys
        )
        ...
        ttk.Button(
            json_buttons_frame, text="Beispiel laden", command=self.load_example_data
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(
            json_buttons_frame, text="Validieren", command=self.validate_json
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(json_buttons_frame, text="Speichern", command=self.save_json).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(json_buttons_frame, text="Laden", command=self.load_json).pack(
            side=tk.LEFT
        )
        ...
        ttk.Radiobutton(
            engine_frame,
            text="Claude (Prompt Design)",
            variable=self.engine_var,
            value="claude",
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Radiobutton(
            engine_frame,
            text="ChatGPT (Text Generierung)",
            variable=self.engine_var,
            value="chatgpt",
        ).pack(side=tk.LEFT)
        ...
        ttk.Button(
            gen_frame, text="Prompt aktualisieren", command=self.update_prompt
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            gen_frame,
            text="Kapitel mit ChatGPT generieren",
            command=self.generate_chapter_with_chatgpt,
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            gen_frame, text="Batch Generierung", command=self.batch_generation_demo
        ).pack(side=tk.LEFT)

        # Ergebnisse
        results_frame = ttk.LabelFrame(
            right_frame, text="Generiertes Kapitel", padding="10"
        )
        ...
        self.results_text = scrolledtext.ScrolledText(
            results_frame, width=60, height=10
        )
    ...
    def setup_story_tab(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="🎭 Story-Struktur & Emotionale Tiefe",
            font=("Arial", 16, "bold"),
        )
        ...
        if not self.claude_client:
            status_label = ttk.Label(
                main_frame,
                text="❌ Claude API Key fehlt - Story-Entwicklung nicht verfügbar",
                font=("Arial", 12, "bold"),
                foreground="red",
            )
        ...
        ttk.Button(
            buttons_frame,
            text="📖 Story-Struktur Vorschläge",
            command=self.get_story_structure_suggestions,
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            buttons_frame,
            text="💝 Emotionale Tiefe Optimierung",
            command=self.optimize_emotional_depth,
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            buttons_frame, text="🎯 Plot-Entwicklung", command=self.develop_plot
        ).pack(side=tk.LEFT)

        # Ergebnisse
        results_frame = ttk.LabelFrame(
            main_frame, text="Claude's Beratung", padding="10"
        )
    ...
    def setup_character_tab(self):
        ...
        # Titel
        title_label = ttk.Label(
            main_frame, text="👤 Charakter-Entwicklung", font=("Arial", 16, "bold")
        )
        title_label.pack(pady=(0, 20))
        ...
        if not self.claude_client:
            status_label = ttk.Label(
                main_frame,
                text="❌ Claude API Key fehlt - Charakter-Entwicklung nicht verfügbar",
                font=("Arial", 12, "bold"),
                foreground="red",
            )
            status_label.pack(pady=(0, 20))
        ...
        character_frame = ttk.LabelFrame(
            main_frame, text="Charakter-Beschreibung", padding="10"
        )
        character_frame.pack(fill=tk.X, pady=(0, 10))

        ttk.Label(character_frame, text="Beschreibe deinen Charakter:").pack(
            anchor=tk.W
        )
        ...
        ttk.Button(
            buttons_frame,
            text="🎭 Charakter-Entwicklung",
            command=self.develop_character,
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            buttons_frame, text="💬 Dialog-Optimierung", command=self.optimize_dialogue
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            buttons_frame, text="🔄 Charakter-Arc", command=self.character_arc
        ).pack(side=tk.LEFT)

        # Ergebnisse
        results_frame = ttk.LabelFrame(
            main_frame, text="Claude's Charakter-Beratung", padding="10"
        )
        results_frame.pack(fill=tk.BOTH, expand=True)
    ...
    def get_story_structure_suggestions(self):
        ...
        try:
            ...
            if not context:
                messagebox.showwarning(
                    "Warnung", "Bitte gib zuerst eine Story-Idee ein"
                )
                return
...
Gib strukturierte, praktische Vorschläge, die sofort umsetzbar sind."""
            ...
            self.story_results_text.insert(
                1.0, "🤔 Claude analysiert deine Story-Struktur...\n\n"
            )
            ...
            messagebox.showerror(
                "Fehler", f"Unerwarteter Fehler bei der Story-Analyse: {e}"
            )
...
Konzentriere dich auf authentische, tiefgreifende Emotionen."""
            ...
            self.story_results_text.delete(1.0, tk.END)
            self.story_results_text.insert(
                1.0, "💝 Claude optimiert die emotionale Tiefe...\n\n"
            )
            self.story_tab.update()
            ...
            logger.error(
                f"Fehler bei der emotionalen Optimierung (Daten/Attribut): {e}"
            )
            messagebox.showerror(
                "Fehler", f"Fehler bei der emotionalen Optimierung: {e}"
            )
        except Exception as e:
            logger.exception(
                f"Unerwarteter Fehler bei der emotionalen Optimierung: {e}"
            )
            messagebox.showerror(
                "Fehler", f"Unerwarteter Fehler bei der emotionalen Optimierung: {e}"
            )
...
Mache den Plot spannend und logisch kohärent."""
            ...
            messagebox.showerror(
                "Fehler", f"Unerwarteter Fehler bei der Plot-Entwicklung: {e}"
            )
    ...
    def develop_character(self):
        ...
        try:
            ...
            if not character:
                messagebox.showwarning(
                    "Warnung", "Bitte gib zuerst eine Charakter-Beschreibung ein"
                )
                return
...
Mache den Charakter komplex, interessant und realistisch."""
            ...
            self.character_results_text.insert(
                1.0, "👤 Claude entwickelt den Charakter...\n\n"
            )
...
Mache die Dialoge authentisch und charakteristisch."""
            ...
            self.character_results_text.delete(1.0, tk.END)
            self.character_results_text.insert(
                1.0, "💬 Claude optimiert die Dialoge...\n\n"
            )
            self.character_tab.update()
...
Mache den Arc überzeugend und bedeutungsvoll."""
            ...
            self.character_results_text.delete(1.0, tk.END)
            self.character_results_text.insert(
                1.0, "🔄 Claude entwickelt den Charakter-Arc...\n\n"
            )
            self.character_tab.update()
            ...
            messagebox.showerror(
                "Fehler", f"Fehler bei der Charakter-Arc-Entwicklung: {e}"
            )
    ...
    def configure_api_keys(self):
        ...
        # Titel
        title_label = ttk.Label(
            main_frame, text="🔑 API Keys konfigurieren", font=("Arial", 14, "bold")
        )
        title_label.pack(pady=(0, 20))
        ...
        # Claude API Key
        claude_frame = ttk.LabelFrame(
            main_frame, text="Anthropic API Key", padding="10"
        )
            ...
            try:
                ...
                if self.anthropic_api_key and len(self.anthropic_api_key.strip()) > 0:
                    try:
                        ...
                        logger.error(
                            f"Claude Client Initialisierung fehlgeschlagen: {e}"
                        )
                        self.claude_client = None

                messagebox.showinfo(
                    "Erfolg", "API Keys wurden erfolgreich gespeichert!"
                )
                config_window.destroy()

            except Exception as e:
                messagebox.showerror(
                    "Fehler", f"Fehler beim Speichern der API Keys: {e}"
                )

        # Größere, auffälligere Buttons
        save_button = ttk.Button(
            button_frame, text="💾 SPEICHERN", command=save_api_keys
        )
        save_button.pack(side="right", padx=(10, 0), pady=10)

        cancel_button = ttk.Button(
            button_frame, text="❌ ABBRECHEN", command=config_window.destroy
        )
        ...
        info_text = ttk.Label(
            main_frame,
            text="💡 API Keys: OpenAI (platform.openai.com) | Anthropic (console.anthropic.com)",
            font=("Arial", 9),
            foreground="gray",
        )
        ...
        try:
            ...
            if not is_valid:
                ...
                if validate_prompt_structure(data):
                    ...
                    messagebox.showerror(
                        "Validierungsfehler", "Ungültige Prompt-Struktur"
                    )
                    self.status_var.set("Validierung fehlgeschlagen")
    ...
    def generate_chapter_with_chatgpt(self):
        ...
        try:
            ...
            if errors:
                messagebox.showerror(
                    "Validierungsfehler", "Bitte korrigiere die JSON-Eingabe"
                )
                return
            ...
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein erfahrener Autor und Schriftsteller.",
                    },
            ...
            messagebox.showerror(
                "API Fehler", "Rate Limit erreicht. Bitte warte einen Moment."
            )
    ...
    def batch_generation_demo(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Batch-Generierung", "Demo: 3 Kapitel wurden generiert!"
            )
