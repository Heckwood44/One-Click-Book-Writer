...
def test_production_framework():
        ...
        try:
            ...
            if drift_alerts:
                ...
                for alert in drift_alerts:
                    print(f"   - {alert.drift_type.value}: {alert.severity} (Confidence: {alert.confidence:.3f})")
            else:
                print(f"✅ Keine Drift erkannt")
            
            # 4. Feedback-Intelligence
            if result.feedback_entries:
                features = feedback_intelligence.analyze_feedback(result.feedback_entries)
                print(f"🧠 Feedback-Features: {len(features)} extrahiert")
                for feature in features:
                    print(f"   - {feature.feature_name}: {feature.sentiment} (Confidence: {feature.confidence:.3f})")
            ...
            if policy_engine.should_start_experiment(segment):
                ...
                for suggestion in suggestions:
                    print(f"   - {suggestion['type']}: {suggestion['description']} ({suggestion['priority']})")
            ...
            if result.ab_test_result:
                improvement = result.ab_test_result.comparison.get('improvement_percentage', 0)
    ...
    generate_production_report(results, policy_engine, drift_detector, feedback_intelligence)

def generate_production_report(results, policy_engine, drift_detector, feedback_intelligence):
    ...
    print(f"\n🚀 PIPELINE-STATISTIKEN:")
    print(f"   Gesamte Runs: {pipeline_stats['total_runs']}")
    print(f"   Erfolgreiche Runs: {pipeline_stats['successful_runs']}")
    print(f"   Erfolgsrate: {(pipeline_stats['successful_runs'] / max(pipeline_stats['total_runs'], 1)) * 100:.1f}%")
    print(f"   Durchschnittliche Ausführungszeit: {pipeline_stats['average_execution_time']:.2f}s")
    print(f"   Gesamtkosten: ${pipeline_stats['total_cost']:.4f}")
