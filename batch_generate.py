...
class BatchGenerator:
    ...
    def _create_chapter_prompt_frame(self, chapter_num: int) -> Dict[str, Any]:
        ...
        if "title" in prompt_frame["input"]["chapter"]:
            ...
            prompt_frame["input"]["chapter"]["title"] = f"Kapitel {chapter_num}: {base_title}"
        ...
        if "story_context" in prompt_frame["input"]:
            ...
            if "current_scene" in story_context:
                ...
                story_context["current_scene"] = f"Kapitel {chapter_num} beginnt mit einer neuen Entwicklung in der Geschichte."
            
            if "previous_summary" in story_context:
                # Vorherige Zusammenfassung anpassen
                story_context["previous_summary"] = f"Nach den Ereignissen von Kapitel {chapter_num - 1} entwickelt sich die Geschichte weiter."
    ...
    def _generate_single_chapter(self, chapter_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            simulated_text = self._generate_simulated_text(chapter_num, metadata, engine)
            
            # Ergebnis speichern
            output_path = self._save_chapter_result(simulated_text, metadata, engine, chapter_num)
    ...
    def _generate_simulated_text(self, chapter_num: int, metadata: Dict[str, Any], engine: str) -> str:
        """
...
Am Ende des Kapitels steht eine neue Wendung, die die Leser neugierig auf das nächste Kapitel macht."""
    ...
    def _save_chapter_result(self, text: str, metadata: Dict[str, Any], engine: str, chapter_num: int) -> str:
        """
    ...
    def save_batch_summary(self, results: List[Dict[str, Any]], output_file: str = None):
        """
        ...
        summary = {
            "batch_info": {
                ...
                "successful_chapters": len([r for r in results if r.get("success", False)]),
                "failed_chapters": len([r for r in results if not r.get("success", False)]),
                "errors": self.errors
            },
            "chapters": results,
            "timestamp": time.time()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="One Click Book Writer - Batch Generation")
    parser.add_argument("--input", "-i", required=True, help="Pfad zur JSON-Eingabedatei")
    parser.add_argument("--start", "-s", type=int, default=1, help="Start-Kapitelnummer")
    parser.add_argument("--count", "-c", type=int, required=True, help="Anzahl der Kapitel")
    parser.add_argument("--engine", "-e", choices=["claude", "chatgpt"], default="claude", help="AI-Engine")
    parser.add_argument("--temperature", "-t", type=float, default=0.4, help="Temperature")
    parser.add_argument("--max-tokens", type=int, default=8000, help="Maximale Anzahl Tokens")
    parser.add_argument("--workers", "-w", type=int, default=3, help="Anzahl paralleler Threads")
