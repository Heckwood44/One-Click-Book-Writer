...
class EnhancedBookWriterGUI:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="One Click Book Writer - Enhanced GUI",
            font=("Arial", 16, "bold"),
        )
        ...
        self.json_text = scrolledtext.ScrolledText(
            input_frame, width=50, height=30, font=("Consolas", 10)
        )
        ...
        control_frame = ttk.LabelFrame(
            main_frame, text="Pipeline-Steuerung", padding="5"
        )
        ...
        chapter_spin = ttk.Spinbox(
            chapter_frame, from_=1, to=50, textvariable=self.chapter_var, width=10
        )
        ...
        ttk.Checkbutton(
            control_frame, text="Claude-Optimierung", variable=self.claude_var
        ...
        ttk.Button(
            control_frame,
            text="Batch-Generierung (3 Kapitel)",
            command=self.batch_generation,
        ).grid(row=3, column=0, sticky=(tk.W, tk.E), pady=(0, 10))

        # API-Keys konfigurieren
        ttk.Button(
            control_frame,
            text="API-Keys konfigurieren",
        ...
        status_frame.grid(
            row=5, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(10, 0)
        )
        status_frame.columnconfigure(0, weight=1)
        status_frame.rowconfigure(0, weight=1)

        self.status_display = scrolledtext.ScrolledText(
            status_frame, width=30, height=10, font=("Consolas", 9)
        )
        self.status_display.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))

        # Rechte Spalte - Ergebnisse
        results_frame = ttk.LabelFrame(
            main_frame, text="Generierte Kapitel", padding="5"
        )
        results_frame.grid(
        ...
        self.result_notebook.grid(
            row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 5)
        )

        # Deutsche Version
        self.de_text = scrolledtext.ScrolledText(
            self.result_notebook, width=50, height=25, font=("Arial", 11)
        )
        ...
        self.meta_text = scrolledtext.ScrolledText(
            self.result_notebook, width=50, height=25, font=("Consolas", 9)
        )
        ...
        ttk.Button(
            result_buttons_frame, text="Alle speichern", command=self.save_all_results
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(
            result_buttons_frame, text="Metadaten anzeigen", command=self.show_metadata
        ).pack(side=tk.LEFT)
        ...
        if os.path.exists(template_file):
            try:
                ...
                self.json_text.insert(
                    1.0, json.dumps(template_data, indent=2, ensure_ascii=False)
                )
    ...
    def load_default_template(self):
        ...
        try:
            with open(
                "data/generate_chapter_full_extended.json", "r", encoding="utf-8"
            ) as f:
                template_data = json.load(f)
    ...
    def validate_json(self):
        ...
        try:
            ...
            (
                ...
                data,
                message,
            ) = self.router.load_and_validate_prompt_frame_from_data(json_data)
    ...
    def _generate_chapter_thread(self):
        ...
        try:
            ...
            error_msg = str(e)
            self.root.after(
                0, lambda: self._show_error(f"Generierungsfehler: {error_msg}")
            )
    ...
    def _show_results(self, result):
        ...
        try:
            if result["success"]:
                ...
                quality_score = result.get("quality_evaluation", {}).get(
                    "overall_bilingual_score", "N/A"
                )
                self.status_var.set(
                    f"✅ Kapitel generiert! Qualitäts-Score: {quality_score}"
                )
                ...
                messagebox.showerror(
                    "Fehler", f"Generierung fehlgeschlagen:\n{error_msg}"
                )
        ...
        try:
            ...
            for chapter_num in range(1, 4):
                ...
                if result["success"]:
                    ...
                    self.log_status(
                        f"Kapitel {chapter_num} erfolgreich (Score: {quality_score})"
                    )
                else:
            ...
            self.root.after(
                ...
                lambda: messagebox.showerror(
                    "Fehler", f"Batch-Generierungsfehler: {error_msg}"
                ),
            )
    ...
    def configure_api_keys(self):
        ...
        ttk.Button(button_frame, text="Speichern", command=save_keys).pack(
            side=tk.RIGHT, padx=(5, 0)
        )
        ttk.Button(button_frame, text="Abbrechen", command=dialog.destroy).pack(
            side=tk.RIGHT
        )
    ...
    def save_all_results(self):
        ...
        try:
            ...
            if meta_content.strip():
                with open(
                    f"{results_dir}/chapter_meta.json", "w", encoding="utf-8"
                ) as f:
                    f.write(meta_content)

            messagebox.showinfo(
                "Erfolg", f"Alle Ergebnisse gespeichert in: {results_dir}"
            )
